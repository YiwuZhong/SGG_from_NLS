...
class TransformerContext(nn.Module):
    def __init__(self, config, obj_classes, rel_classes, in_channels):
        ...
        self.in_channels = in_channels if not self.cfg.WSVL.OFFLINE_OD else self.cfg.MODEL.ROI_RELATION_HEAD.CONTEXT_POOLING_DIM
        ...
        with torch.no_grad():
            if self.cfg.WSVL.OFFLINE_OD:  # offline object detector
                if config.WSVL.OFFLINE_OD_TYPE == 'Updown':
                    category_file = "objects_vocab.txt" 
                elif config.WSVL.OFFLINE_OD_TYPE == 'OID': 
                    category_file = "oid_v4_vocab.txt"
                with open(category_file, 'r') as f:
                    bup_classes = []
                    for item in f:
                        bup_classes.append(str(item.split(",")[0].strip()))
                offline_obj_embed_vecs = obj_edge_vectors(bup_classes, wv_dir=self.cfg.GLOVE_DIR, wv_dim=self.embed_dim, offline_od=True)
    ...
    def forward(self, roi_features, proposals, logger=None, det_dists=None):
        ...
        if self.mode == 'predcls':
            ...
            use_decoder_nms = self.mode == 'sgdet' and not self.training  # use post nms when sgdet test
            if not self.cfg.WSVL.OFFLINE_OD: # online object detector
                if use_decoder_nms:
                ...
                # disable the decoder nms when using offline detector for now
                obj_preds = obj_dists[:, 1:].max(1)[1] + 1
