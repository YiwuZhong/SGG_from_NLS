...
from .utils_uniter import BertLayer, BertPooler
...
class UniterPreTrainedModel(nn.Module):
    ...
    def init_weights(self, module):
        ...
        if isinstance(module, (nn.Linear, nn.Embedding)):
            if module.weight.shape[0] != 605: # GLove emb
    ...
    @classmethod
    def from_pretrained(cls, config_file='./datasets/vg/uniter-base.json', state_dict={}, *inputs, **kwargs):
        """
        ...
        """
        load_pretrained = False if state_dict == {} else True
        # Load config
        config = UniterConfig.from_json_file(config_file)
        logger.info("#################### load pretrained Uniter ####################")
        logger.info("Pretrained Uniter model config {}".format(config))
        ...
        old_keys = []
        new_keys = []
        # with open('uniter_base_ckpt_keys.txt','w') as f:
        #     for key in state_dict.keys():
        #         f.write(key + '\n')
        for key in state_dict.keys():
            new_key = None
            if 'uniter' in key:
                new_key = key.replace('uniter.', '')  # remove the prefix in loaded state_dict
            if 'gamma' in key:
...
class UniterTextEmbeddings(nn.Module):
    def __init__(self, config, offline_od=False):
        ...
        if self.glove_emb_layer:
            ...
            with torch.no_grad():
                    ...
                    offline_obj_embed_vecs = obj_edge_vectors(detobj_classes, wv_dir=wv_dir, wv_dim=self.embed_dim, offline_od=True)
                    self.word_embeddings = nn.Embedding(offline_obj_embed_vecs.shape[0] + 3, self.embed_dim)  # additional tokens such as mask
                    offline_obj_embed_vecs = torch.cat([offline_obj_embed_vecs, self.word_embeddings.weight.data[-3:,:]], dim=0)
                    #self.word_embeddings.weight.data[:offline_obj_embed_vecs.shape[0]] = offline_obj_embed_vecs
                    self.word_embeddings.weight.copy_(offline_obj_embed_vecs, non_blocking=True)
            self.wd_emb_proj = nn.Linear(self.embed_dim, config.hidden_size)
            ##### Glove embedding #####     
        else: 
            self.word_embeddings = nn.Embedding(config.vocab_size,
                                                config.hidden_size, padding_idx=0) # [28996,768]
        self.position_embeddings = nn.Embedding(config.max_position_embeddings,
                                                config.hidden_size) # [512,768]
        self.token_type_embeddings = nn.Embedding(config.type_vocab_size,
                                                  config.hidden_size) # [2,768]
    ...
    def forward(self, input_ids, position_ids, token_type_ids=None):
        ...
        words_embeddings = words_embeddings if not self.glove_emb_layer else self.wd_emb_proj(words_embeddings)
        ...
        embeddings = self.LayerNorm(embeddings.type(self.LayerNorm.weight.dtype))
...
class UniterImageEmbeddings(nn.Module):
    ...
    def forward(self, img_feat, img_pos_feat, type_embeddings, img_masks=None):
        ...
        transformed_im = self.img_layer_norm(self.img_linear(img_feat).type(self.LayerNorm.weight.dtype))
        transformed_pos = self.pos_layer_norm(self.pos_linear(img_pos_feat).type(self.LayerNorm.weight.dtype))
...
class UniterModel(UniterPreTrainedModel):
    ...
    """
    def __init__(self, config, img_dim, offline_od=False):
        super().__init__(config)
        self.embeddings = UniterTextEmbeddings(config, offline_od=offline_od)
