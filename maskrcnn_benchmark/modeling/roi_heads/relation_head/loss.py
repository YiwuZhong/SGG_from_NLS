...
class RelationLossComputation(object):
    ...
    def __init__(
        ...
        if self.use_label_smoothing:
            self.criterion_loss = Label_Smoothing_Regression(e=0.01) # larger e means smaller value for the "one" in one-hot vector
        else:
            if loss_cls_weights is not None: # use the loss weights for caption-supervised setting
                weight_file = loss_cls_weights
                precompute_weights = np.load(weight_file, allow_pickle=True).tolist()
                pred_weights = torch.tensor(precompute_weights["relations"]).type(torch.float16).cuda()
                obj_weights = torch.tensor(precompute_weights["objects"]).type(torch.float16).cuda()
                self.pred_criterion_loss = nn.CrossEntropyLoss(reduction='mean', weight=pred_weights)
                self.criterion_loss = nn.CrossEntropyLoss(reduction='mean', weight=obj_weights)
            else:  # default
                self.pred_criterion_loss = nn.CrossEntropyLoss(reduction='mean')
                self.criterion_loss = nn.CrossEntropyLoss(reduction='mean')
    ...
    def __call__(self, proposals, rel_labels, relation_logits, refine_logits):
        ...
        loss_relation = self.pred_criterion_loss(relation_logits, rel_labels.long())
...
class UniterLossComputation(RelationLossComputation):
    """
    Computes the loss for relation triplet.
    Also supports FPN
    """
    ...
    def __init__(
        ...
        super(UniterLossComputation, self).__init__(
        attri_on,
        num_attri_cat,
        max_num_attri,
        attribute_sampling,
    ...
    def __call__(self, proposals, rel_labels, relation_logits, object_logits, rel_pair_idxs):
        """
        Different from RelationLossComputation.__call__ which calcuates loss for detected objects and relations,
        this class computes loss for subject, object and relation respectively.
        ...
        Returns:
            ...
            finetune_obj_loss (Tensor)
        """
        sub_logits, obj_logits = object_logits
    
        relation_logits = cat(relation_logits, dim=0)
        
        subj_labels = cat([proposals[bs_i].get_field("labels")[rel_pair_idxs[bs_i][:,0]] for bs_i in range(len(proposals))], dim=0)
        oobj_labels = cat([proposals[bs_i].get_field("labels")[rel_pair_idxs[bs_i][:,1]] for bs_i in range(len(proposals))], dim=0)
        rel_labels = cat(rel_labels, dim=0)
        # only calculate the rel loss for the rel MASK whose head and tail are both fg regions
        fg_pair_ind = torch.nonzero(torch.logical_and(subj_labels != 0, oobj_labels != 0)).view(-1).contiguous()
        relation_logits = relation_logits[fg_pair_ind]
        rel_labels = rel_labels[fg_pair_ind]
        
        loss_relation = self.pred_criterion_loss(relation_logits, rel_labels.long())
        loss_refine_obj = self.criterion_loss(sub_logits, subj_labels.long())
        loss_refine_obj += self.criterion_loss(obj_logits, oobj_labels.long())
        loss_refine_obj = loss_refine_obj * 0.5

        return loss_relation, loss_refine_obj    


def make_roi_relation_loss_evaluator(cfg):
    if not cfg.WSVL.USE_UNITER:  # use default relation predictor
        loss_evaluator = RelationLossComputation(
            ...
            loss_cls_weights=cfg.WSVL.LOSS_CLS_WEIGHTS,
        )
    else:  # use uniter as relation predictor
        loss_evaluator = UniterLossComputation(
            cfg.MODEL.ATTRIBUTE_ON,
            cfg.MODEL.ROI_ATTRIBUTE_HEAD.NUM_ATTRIBUTES,
            cfg.MODEL.ROI_ATTRIBUTE_HEAD.MAX_ATTRIBUTES,
            cfg.MODEL.ROI_ATTRIBUTE_HEAD.ATTRIBUTE_BGFG_SAMPLE,
