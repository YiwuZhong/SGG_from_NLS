...
class DecoderRNN(nn.Module):
    ...
    def forward(self, inputs, initial_state=None, labels=None, boxes_for_nms=None):
        ...
        if initial_state is None:
            previous_memory = sequence_tensor.new().resize_(batch_size, self.hidden_size).fill_(0) #.type(torch.float16)
            previous_state = sequence_tensor.new().resize_(batch_size, self.hidden_size).fill_(0) #.type(torch.float16)
        else:
        ...
        if self.rnn_drop > 0.0:
            dropout_mask = get_dropout_mask(self.rnn_drop, previous_memory.size(), previous_memory.device) #.type(torch.float16)
        else:
            dropout_mask = None
        ...
        # Do NMS here as a post-processing step (for offline detector, boxes_for_nms is None)
        if boxes_for_nms is not None and not self.training:
            ...
            # post nms: start from the region with highest score (out_dists_sampled.argmax()), and eleminate similar box with same prediction
            for i in range(out_commitments.size(0)):
                box_ind, cls_ind = np.unravel_index(out_dists_sampled.argmax(), out_dists_sampled.shape) # convert to 2d index
...
class LSTMContext(nn.Module):
    ...
    def __init__(self, config, obj_classes, rel_classes, in_channels):
        ...
        with torch.no_grad():
            if self.cfg.WSVL.OFFLINE_OD:  # offline object detector
                category_file = "datasets/vg/oid_v4_vocab.txt"
                with open(category_file, 'r') as f:
                    detobj_classes = []
                    for item in f:
                        detobj_classes.append(str(item.split(",")[0].strip()))
                offline_obj_embed_vecs = obj_edge_vectors(detobj_classes, wv_dir=self.cfg.GLOVE_DIR, wv_dim=self.embed_dim, offline_od=True)
                self.obj_embed1 = nn.Embedding(offline_obj_embed_vecs.shape[0], self.embed_dim)
                self.obj_embed1.weight.copy_(offline_obj_embed_vecs, non_blocking=True)
        ...
        self.obj_dim = in_channels if not self.cfg.WSVL.OFFLINE_OD else self.cfg.MODEL.ROI_RELATION_HEAD.CONTEXT_POOLING_DIM
    ...
    def forward(self, x, proposals, rel_pair_idxs, logger=None, all_average=False, ctx_average=False, det_dists=None):
        ...
        if self.cfg.MODEL.ROI_RELATION_HEAD.USE_GT_OBJECT_LABEL:
            ...
            if not self.cfg.WSVL.OFFLINE_OD:  # online object detector
                obj_logits = cat([proposal.get_field("predict_logits") for proposal in proposals], dim=0).detach()
                obj_embed = F.softmax(obj_logits, dim=1) @ self.obj_embed1.weight
            else:  # offline object detector detection results
                obj_dists = torch.cat(det_dists)
                obj_embed = obj_dists @ self.obj_embed1.weight
        
        assert proposals[0].mode == 'xyxy'
        pos_embed = self.pos_embed(encode_box_info(proposals)) #self.pos_embed(encode_box_info(proposals).type(torch.float16)) # for comparison
